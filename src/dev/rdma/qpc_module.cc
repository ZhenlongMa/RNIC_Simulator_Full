...
HanGuRnic::QpcModule::postQpcReq(CxtReqRspPtr qpcReq) {
    ...
    } else if (qpcReq->chnl == CXT_CHNL_TX && qpcReq->type == CXT_PREF_QP) {
        prefQpcFifo.push(qpcReq);
    } else {
...
HanGuRnic::QpcModule::readProc(uint8_t chnlNum, CxtReqRspPtr qpcReq) {
    ...
    if (qpnHashMap.find(qpcReq->num) != qpnHashMap.end()) { /* related qpn is fond in qpnHashMap */
        qpnHashMap[qpcReq->num]->reqCnt += 1;
         ...
         HANGU_PRINT(CxtResc, " QpcModule.qpcReqProc.readProc: related qpn is found in qpnHashMap ,chnlNum %d, qpn %d, idx %d\n", chnlNum, qpcReq->num, qpcReq->idx);
    ...
    if (qpcCache.lookupHit(qpcReq->num)) { /* cache hit, and return related rsp to related fifo */
        ...
        HANGU_PRINT(CxtResc, " QpcModule.qpcReqProc.readProc: cache hit, chnlNum %d, qpn %d\n", chnlNum, qpcReq->num);
        
        // printf("QpcModule.qpcReqProc.readProc: cache hit,qpn: %d, current_time: %ld\n", qpcReq->num, curTick());
        
        hitProc(chnlNum, qpcReq);
    } else { /* cache miss */

        HANGU_PRINT(CxtResc, " QpcModule.qpcReqProc.readProc: cache miss, chnlNum %d, qpn %d, rtnCnt %d\n", chnlNum, qpcReq->num, rtnCnt);

        // printf("QpcModule.qpcReqProc.readProc: cache miss,qpn: %d, rtnCnt %d, current_time: %ld\n", qpcReq->num, rtnCnt, curTick());
...
HanGuRnic::QpcModule::qpcAccess() {
    ...
    isEmpty[3] = prefQpcFifo.empty();

    //HANGU_PRINT(CxtResc, " QpcModule.qpcReqProc.qpcAccess: empty[0] %d empty[1] %d empty[2] %d empty[3] %d\n", isEmpty[0], isEmpty[1], isEmpty[2], isEmpty[3]);
        ...
        if (isEmpty[this->chnlIdx] == false) {
            switch (this->chnlIdx) {
              case 0:
                ...
                //HANGU_PRINT(CxtResc, "qpcAccess: txQpAddrRreq qp %d, current_time: %ld\n", qpcReq->num, curTick());
              ...
              case 1:
                ...
                //HANGU_PRINT(CxtResc, "qpcAccess: txQpcRreq qp %d, current_time: %ld\n", qpcReq->num, curTick());
              ...
              case 2:
                ...
                //HANGU_PRINT(CxtResc, "qpcAccess: rxQpcRreq qp %d, current_time: %ld\n", qpcReq->num, curTick());

                break;

              case 3:
                qpcReq = prefQpcFifo.front();
                prefQpcFifo.pop();
                assert(qpcReq->chnl == CXT_CHNL_TX && qpcReq->type == CXT_PREF_QP);

                //HANGU_PRINT(CxtResc, "qpcAccess: prefetch qp %d, current_time: %ld\n", qpcReq->num, curTick());
            ...
            // printf("QpcModule.qpcReqProc.qpcAccess: qpn: %d, current_time: %ld\n", qpcReq->num, curTick());
...
HanGuRnic::QpcModule::loadMem(CxtReqRspPtr qpcReq) {
    ...
    //HANGU_PRINT(CxtResc, " QpcModule.loadMem: Post qpn %d to dmaEngine, idx %d, pending size %d, current_time: %ld\n", qpcReq->num, qpcReq->idx, pendStruct.get_size(), curTick());
...
HanGuRnic::QpcModule::qpcRspProc() {
    ...
    if (rnic->qpcDmaRdCplFifo.size()) { /* processing dmaRsp pkt */
        ...
        if (pElem->has_dma) {
            ...
            //HANGU_PRINT(CxtResc, " QpcModule.qpcRspProc: rtnCnt %d get_size() %d, qpnHashMap.size() %d, qInfo->reqCnt %d, qpn %d, current_time %ld\n",  rtnCnt, pendStruct.get_size(), qpnHashMap.size(), qInfo->reqCnt, qpn, curTick());

            // printf("QpcModule.qpcRspProc: dma return , qpn: %d, current_time = %ld\n", qpn, curTick());
